...
### describe "Disk clone integration"
  ...
  - **it "can clone a disk and its content"** — clones the test VM's primary disk
...
### describe "Disk rebase integration (requires qemu-img)"
  ...
  - **it "flattens an overlay into a standalone disk"** — `runFlattenTest`: registers
  ...
  - **it "rebases overlay to a different backing image"** — `runRebaseTest`: registers
...
### describe "Disk import integration (requires qemu-img)"
  ...
  - **it "imports a local file by copying it to destination"** — `runImportCopyTest`:
  ...
  - **it "rejects import when source and destination are the same"** —
...
### describe "Live disk attach/detach integration"
  ...
  - **it "can hot-plug and hot-unplug a data disk on a running VM"** — boots a VM
  ...
  - **it "can attach a disk read-only to a running VM"** — same flow with
...
### describe "Snapshot integration through daemon"
  ...
  - **it "can rollback to restore deleted file"** — boots VM, writes a
  ...
  - **it "rejects duplicate snapshot names on the same disk"** — first
  ...
  - **it "handles concurrent snapshot operations"** — creates `concurrent-1`,
...
### describe "VM integration"
  ...
  - **it "guest-exec: runs commands and executes as root"** —
  ...
  - **it "SSH: runs commands, checks OS, reports failures, gets system info"** —
  ...
  - **it "can edit CPU and RAM of a stopped VM"** — boots with 2 CPU / 2048 MB.
  ...
  - **it "UEFI VM has EFI boot entries"** — `efibootmgr` exits `ExitSuccess`
    and stdout contains `BootOrder`.

  - **it "BIOS VM does not have EFI support"** — same command exits non-zero
    and stderr contains `EFI variables are not supported`.

  - **it "async start/stop works (no --wait)"** — `vmStop` with wait=False
  ...
  - **it "sync start/stop works (--wait)"** — `vmStop` with wait=True; reading
