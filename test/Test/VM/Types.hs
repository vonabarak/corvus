...
-- | Path to pre-baked SSH private key. The keypair is generated by
--   @make test-image-alpine@ next to the baked Alpine image at
--   @~/VMs/BaseImages/Alpine/corvus-test-key@. We resolve $HOME at
--   module load time so the path is stable across test invocations.
prebakedSshKeyPath :: FilePath
prebakedSshKeyPath = unsafePerformIO $ do
  home <- System.Directory.getHomeDirectory
  pure (home </> "VMs" </> "BaseImages" </> "Alpine" </> "corvus-test-key")
{-# NOINLINE prebakedSshKeyPath #-}
