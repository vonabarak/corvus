  ...
  case mDisk of
    ...
    Just disk -> do
      ...
      case mVm of
        ...
        Just vm
          | media == Just MediaCdrom
          , vmStatus vm `elem` [VmStarting, VmRunning, VmPaused] ->
              -- CD-ROM drives cannot be hot-plugged; attach them
              -- only while the VM is stopped.
              pure $
                RespError
                  "Cannot attach a CD-ROM drive while the VM is active; attach a CD-ROM drive only while the VM is stopped, or use 'crv disk media change' on an existing CD-ROM drive"
          | otherwise -> do
...
handleDiskDetach state vmId diskId = runServerLogging state $ do
  ...
  mDrive <-
    liftIO $ runSqlPool (getBy (UniqueDrive (toSqlKey vmId) (Just (toSqlKey diskId)))) (ssDbPool state)
  case mDrive of
      ...
      case mVm of
        ...
        Just vm -> do
          ...
          if vmStatus vm `elem` [VmStarting, VmRunning, VmPaused]
            then
              if not (supportsLiveDetach (driveInterface drive) (driveMedia drive))
                then
                  pure $
                    RespError $
                      "Cannot detach a "
...
-- interfaces must be detached after the VM has stopped. CD-ROM
-- drives are never live-detachable: their media is swapped with
-- 'crv disk media eject'\/'change' instead.
supportsLiveDetach :: DriveInterface -> Maybe DriveMedia -> Bool
supportsLiveDetach _ (Just MediaCdrom) = False
supportsLiveDetach InterfaceVirtio _ = True
supportsLiveDetach InterfaceScsi _ = True
supportsLiveDetach _ _ = False
